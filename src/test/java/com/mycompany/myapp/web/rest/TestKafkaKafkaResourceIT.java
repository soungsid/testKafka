...
class TestKafkaKafkaResourceIT {
    ...
    void setup() {
        ...
        TestKafkaKafkaResource kafkaResource = new TestKafkaKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/test-kafka-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/test-kafka-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
